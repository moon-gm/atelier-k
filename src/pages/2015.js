...
import Config from "../config/config_gallery-2015.js"
import Styles from "../sass/module/pages/common-gallery.module.scss"

const Gallery2015 = () => (
  <Layout>
    <SEO title="Gallery 2015" />
    ...
    <div className="flex-all flex-wrap">
        {/* ループでジャンプする月別フォルダを作成 */
            Config.map ( month => { return (
                ...
                >
                    <Link
                        to={ `${ process.env.APP_NAME }` !== "" ? `#${ month[0].month }` : `/2015#${ month[0].month }` }
                    >
                ...
                >
                    {// コンテンツ単位のループ
                        month.map( contents => { return (
                            ...
                            >
                                <Link
                                    to={ `${ process.env.APP_NAME }` !== "" ? `${ process.env.APP_NAME }/images/2015/${ contents.month }/${ contents.fileName }` : `/images/2015/${ contents.month }/${ contents.fileName }` }
                                    target="_blank"
                                    rel="noreferrer"
                                >
