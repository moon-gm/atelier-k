...
import Config from "../config/config_gallery-2019.js"
import Styles from "../sass/module/pages/common-gallery.module.scss"

const Gallery2019 = () => (
  <Layout>
    <SEO title="Gallery 2019" />
    ...
    <div className="flex-all flex-wrap">
        {/* ループでジャンプする月別フォルダを作成 */
            Config.map ( month => { return (
                ...
                >
                    <Link
                        to={ `${ process.env.APP_NAME }` !== "" ? `#${ month[0].month }` : `/2019#${ month[0].month }` }
                    >
                ...
                >
                    {// コンテンツ単位のループ
                        month.map( contents => { return (
                            ...
                            >
                                <Link
                                    to={ `${ process.env.APP_NAME }` !== "" ? `${ process.env.APP_NAME }/images/2019/${ contents.month }/${ contents.fileName }` : `/images/2019/${ contents.month }/${ contents.fileName }` }
                                    target="_blank"
                                    rel="noreferrer"
                                >
